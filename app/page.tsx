    ...
    <main>
      ...
      <section className="hero">
        ...
        <div className="hero-copy">
          ...
          <div className="hero-actions"><Link className="button" href="/diagnostico">Diagnosticar minha gestão</Link><Link className="text-link" href="/sobre-o-metodo"><i>▶</i> Entenda o método</Link></div>
      ...
      <section className="category-strip" aria-label="Categorias analisadas"><p>O QUE ANALISAMOS</p><div className="category-list">{categories.map((category) => <Link key={category.slug} href={`/categorias/${category.slug}`}><span>{category.index}</span>{category.shortTitle}<b>Ver</b></Link>)}</div></section>
      ...
      <section className="method-band">
        <div className="method-copy"><p className="kicker">DIAGNÓSTICO ANTES DE PRODUTO</p><h2>Uma decisão melhor começa com a pergunta certa.</h2><p>Nosso método organiza a conversa em três movimentos simples. Só depois o mercado entra na análise.</p><Link className="text-link dark-link" href="/sobre-o-metodo">Conhecer o método</Link></div>
      ...
      <section className="category-showcase content-section">
        ...
        <div className="showcase-grid">{categories.map((category) => <Link key={category.slug} href={`/categorias/${category.slug}`} className="showcase-card"><img src={category.image} alt={category.imageAlt} /><span>{category.index}</span><h3>{category.title}</h3><p>{category.description}</p><b>Explorar categoria</b></Link>)}</div>
      </section>
      ...
      <section className="closing-section"><p className="kicker">DECISÕES SEM ACHISMO</p><h2>Antes de escolher o próximo benefício, entenda o que precisa mudar.</h2><Link className="button" href="/diagnostico">Começar pelo diagnóstico</Link></section>
      <SiteFooter />
    </main>
  );
}
